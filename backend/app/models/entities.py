...
from sqlalchemy import Boolean, CheckConstraint, DateTime, Enum as SqlEnum, ForeignKey, ForeignKeyConstraint, Index, Integer, JSON, String, Text, UniqueConstraint, Uuid
...
class Team(TimestampMixin, Base):
    ...
    college_type: Mapped[CollegeType] = mapped_column(SqlEnum(CollegeType, name="college_type", values_callable=lambda x: [e.value for e in x]), nullable=False)
    college_name: Mapped[str | None] = mapped_column(String(200))
    member_count: Mapped[int] = mapped_column(Integer, nullable=False)
    status: Mapped[RegistrationStatus] = mapped_column(SqlEnum(RegistrationStatus, name="registration_status", values_callable=lambda x: [e.value for e in x]), default=RegistrationStatus.SUBMITTED, nullable=False)
...
class TeamMember(TimestampMixin, Base):
    ...
    __table_args__ = (
        ...
        UniqueConstraint("team_id", "member_number", name="uq_team_members_team_member_number"),
        UniqueConstraint("hackathon_id", "euphoria_id_normalized", name="uq_team_members_hackathon_euphoria"),
        UniqueConstraint("hackathon_id", "email_normalized", name="uq_team_members_hackathon_email"),
        UniqueConstraint("hackathon_id", "phone_normalized", name="uq_team_members_hackathon_phone"),
        UniqueConstraint("hackathon_id", "registration_number_normalized", name="uq_team_members_hackathon_registration"),
        CheckConstraint("member_number BETWEEN 1 AND 5", name="ck_team_members_member_number"),
        CheckConstraint("(member_number = 1 AND is_team_lead IS TRUE) OR (member_number > 1 AND is_team_lead IS FALSE)", name="ck_team_members_lead_position"),
        ...
        Index("ix_team_members_name", "name"),
        Index("ix_team_members_email_normalized", "email_normalized"),
        Index("ix_team_members_phone_normalized", "phone_normalized"),
        Index("ix_team_members_euphoria_id_normalized", "euphoria_id_normalized"),
        Index("ix_team_members_registration_number_normalized", "registration_number_normalized"),
    )
    id: Mapped[uuid.UUID] = mapped_column(Uuid, primary_key=True, default=uuid.uuid4)
    team_id: Mapped[uuid.UUID] = mapped_column(Uuid, nullable=False)
    hackathon_id: Mapped[uuid.UUID] = mapped_column(Uuid, nullable=False)
    ...
    accommodation_type: Mapped[AccommodationType | None] = mapped_column(SqlEnum(AccommodationType, name="accommodation_type", values_callable=lambda x: [e.value for e in x]))
...
class Admin(TimestampMixin, Base):
    ...
    role: Mapped[AdminRole] = mapped_column(SqlEnum(AdminRole, name="admin_role", values_callable=lambda x: [e.value for e in x]), default=AdminRole.ADMIN, nullable=False)
    is_active: Mapped[bool] = mapped_column(Boolean, default=True, nullable=False)
